// Bahnasy Tree - AND Queries
// Author: Mostafa Bahnasy
// License: MIT
// Note: Range AND update not supported (AND has no inverse)

#include<bits/stdc++.h>
using namespace std;
typedef long long ll;
const int MAX_N = 2e5+5;
const ll ALL_BITS = (1LL<<60)-1;
...
struct Node{
    ...
    inline void calc(){ a=ALL_BITS; for(auto c:ch) a&=c->a; }
    
    inline void pref(){
        int n=ch.size();
        pa.assign(n+1,ALL_BITS); pz.assign(n+1,0);
        for(int i=0;i<n;i++) pa[i+1]=pa[i]&ch[i]->a, pz[i+1]=pz[i]+ch[i]->sz;
    }
    ...
    bool ins(int i,ll v){
        i=max(1,min(i,sz+1)); sz++;
        if(!ch.empty()&&ch[0]->ch.empty()){
            Node* nd=new Node(1); nd->a=v;
    ...
    ll qry(int l,int r){
        if(ch.empty()||l>sz||r<1) return ALL_BITS;
        l=max(1,l); r=min(r,sz);
        if(l>r) return ALL_BITS;
        if(l==1&&r==sz) return a;
        if(ch[0]->ch.empty()){
            ll res=ALL_BITS;
            for(int i=l,e=min(r,(int)ch.size());i<=e;i++) res&=ch[i-1]->a;
            return res;
        }
        int lc=find(l),rc=find(r);
        if(lc==rc) return ch[lc]->qry(l-pz[lc],r-pz[lc]);
        ll res=ch[lc]->qry(l-pz[lc],ch[lc]->sz);
        for(int i=lc+1;i<rc;i++) res&=ch[i]->a;
...
void collect(Node* nd,vector<ll>& a){
    ...
    if(nd->ch[0]->ch.empty())
        for(auto c:nd->ch) a.push_back(c->a);
    else
...
void build(vector<ll>& a){
    ...
    function<void(Node*,int&)> dfs=[&](Node* nd,int& i){
        if(nd->ch.empty()) return;
        if(nd->ch[0]->ch.empty()){
            for(auto c:nd->ch) if(i<a.size()) c->a=a[i++];
            nd->calc(); nd->pref();
        }else{
