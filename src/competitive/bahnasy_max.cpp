// Bahnasy Tree - Maximum Queries
// Author: Mostafa Bahnasy
// License: MIT
// Performance: ~1500ms on 100k elements, 50k operations
...
struct Node{
    ...
    Node(int n=0): sz(n), mx(NINF), lz(0){}
    ...
    inline void calc(){ mx=NINF; for(auto c:ch) mx=max(mx,c->mx); }
    
    inline void pref(){
        int n=ch.size();
        pm.assign(n+1,NINF); pz.assign(n+1,0);
        for(int i=0;i<n;i++) pm[i+1]=max(pm[i],ch[i]->mx), pz[i+1]=pz[i]+ch[i]->sz;
    }
    
    inline void push(){
        if(!lz||ch.empty()) return;
        for(auto c:ch) c->lz+=lz, c->mx+=lz;
        lz=0; pref();
    }
    ...
    ll qry(int l,int r){
        if(ch.empty()||l>sz||r<1) return NINF;
        l=max(1,l); r=min(r,sz);
        if(l>r) return NINF;
        if(l==1&&r==sz) return mx;
        push();
        if(ch[0]->ch.empty()){
            ll res=NINF;
            for(int i=l,e=min(r,(int)ch.size());i<=e;i++) res=max(res,ch[i-1]->mx);
            return res;
        }
        int lc=find(l),rc=find(r);
        if(lc==rc) return ch[lc]->qry(l-pz[lc],r-pz[lc]);
        ll res=ch[lc]->qry(l-pz[lc],ch[lc]->sz);
        for(int i=lc+1;i<rc;i++) res=max(res,ch[i]->mx);
        return max(res,ch[rc]->qry(1,r-pz[rc]));
    }
    ...
    void add(int l,int r,ll d){
        ...
        l=max(1,l); r=min(r,sz);
        if(l>r) return;
        if(l==1&&r==sz){ mx+=d; lz+=d; return; }
        push();
        if(ch[0]->ch.empty()){
            for(int i=l,e=min(r,(int)ch.size());i<=e;i++) ch[i-1]->mx+=d;
            calc(); pref(); return;
        }
        int lc=find(l),rc=find(r);
...
void collect(Node* nd,vector<ll>& a){
    ...
    nd->push();
    if(nd->ch[0]->ch.empty())
        for(auto c:nd->ch) a.push_back(c->mx);
    else
...
void build(vector<ll>& a){
    ...
    function<void(Node*,int&)> dfs=[&](Node* nd,int& i){
        if(nd->ch.empty()) return;
        if(nd->ch[0]->ch.empty()){
            for(auto c:nd->ch) if(i<a.size()) c->mx=a[i++];
            nd->calc(); nd->pref();
        }else{
