// Bahnasy Tree - XOR Queries
// Author: Mostafa Bahnasy
// License: MIT
// Note: Range XOR update not supported (XOR has no inverse for lazy prop)
...
struct Node{
    ...
    inline void calc(){ x=0; for(auto c:ch) x^=c->x; }
    
    inline void pref(){
        int n=ch.size();
        px.assign(n+1,0); pz.assign(n+1,0);
        for(int i=0;i<n;i++) px[i+1]=px[i]^ch[i]->x, pz[i+1]=pz[i]+ch[i]->sz;
    }
    ...
    ll qry(int l,int r){
        ...
        if(l>r) return 0;
        if(l==1&&r==sz) return x;
        if(ch[0]->ch.empty()){
            ll res=0;
            for(int i=l,e=min(r,(int)ch.size());i<=e;i++) res^=ch[i-1]->x;
            return res;
        }
        int lc=find(l),rc=find(r);
        if(lc==rc) return ch[lc]->qry(l-pz[lc],r-pz[lc]);
        ll res=ch[lc]->qry(l-pz[lc],ch[lc]->sz);
        for(int i=lc+1;i<rc;i++) res^=ch[i]->x;
...
void collect(Node* nd,vector<ll>& a){
    ...
    if(nd->ch[0]->ch.empty())
        for(auto c:nd->ch) a.push_back(c->x);
    else
...
void build(vector<ll>& a){
    ...
    function<void(Node*,int&)> dfs=[&](Node* nd,int& i){
        if(nd->ch.empty()) return;
        if(nd->ch[0]->ch.empty()){
            for(auto c:nd->ch) if(i<a.size()) c->x=a[i++];
            nd->calc(); nd->pref();
        }else{
