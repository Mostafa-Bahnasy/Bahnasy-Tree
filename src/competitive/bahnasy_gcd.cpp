// Bahnasy Tree - GCD Queries
// Author: Mostafa Bahnasy
// License: MIT
// Note: Range add not supported for GCD (would break GCD property)
...
struct Node{
    ...
    inline void calc(){ g=0; for(auto c:ch) g=gcd(g,c->g); }
    
    inline void pref(){
        int n=ch.size();
        pg.assign(n+1,0); pz.assign(n+1,0);
        for(int i=0;i<n;i++) pg[i+1]=gcd(pg[i],ch[i]->g), pz[i+1]=pz[i]+ch[i]->sz;
    }
    ...
    ll qry(int l,int r){
        ...
        if(l>r) return 0;
        if(l==1&&r==sz) return g;
        if(ch[0]->ch.empty()){
            ll res=0;
            for(int i=l,e=min(r,(int)ch.size());i<=e;i++) res=gcd(res,ch[i-1]->g);
            return res;
        }
        int lc=find(l),rc=find(r);
        if(lc==rc) return ch[lc]->qry(l-pz[lc],r-pz[lc]);
        ll res=ch[lc]->qry(l-pz[lc],ch[lc]->sz);
        for(int i=lc+1;i<rc;i++) res=gcd(res,ch[i]->g);
        return gcd(res,ch[rc]->qry(1,r-pz[rc]));
    }
    
    void upd(int i,ll v){
        if(ch.empty()||i<1||i>sz) return;
        if(ch[0]->ch.empty()){
            if(i<=ch.size()) ch[i-1]->g=v;
            calc(); pref(); return;
        }
...
void collect(Node* nd,vector<ll>& a){
    ...
    if(nd->ch[0]->ch.empty())
        for(auto c:nd->ch) a.push_back(c->g);
    else
...
void build(vector<ll>& a){
    ...
    function<void(Node*,int&)> dfs=[&](Node* nd,int& i){
        if(nd->ch.empty()) return;
        if(nd->ch[0]->ch.empty()){
            for(auto c:nd->ch) if(i<a.size()) c->g=a[i++];
            nd->calc(); nd->pref();
        }else{
