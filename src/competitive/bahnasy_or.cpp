// Bahnasy Tree - OR Queries
// Author: Mostafa Bahnasy
// License: MIT
// Note: Range OR update not supported (OR has no inverse)
...
struct Node{
    ...
    inline void calc(){ o=0; for(auto c:ch) o|=c->o; }
    
    inline void pref(){
        int n=ch.size();
        po.assign(n+1,0); pz.assign(n+1,0);
        for(int i=0;i<n;i++) po[i+1]=po[i]|ch[i]->o, pz[i+1]=pz[i]+ch[i]->sz;
    }
    ...
    ll qry(int l,int r){
        ...
        if(l>r) return 0;
        if(l==1&&r==sz) return o;
        if(ch[0]->ch.empty()){
            ll res=0;
            for(int i=l,e=min(r,(int)ch.size());i<=e;i++) res|=ch[i-1]->o;
            return res;
        }
        int lc=find(l),rc=find(r);
        if(lc==rc) return ch[lc]->qry(l-pz[lc],r-pz[lc]);
        ll res=ch[lc]->qry(l-pz[lc],ch[lc]->sz);
        for(int i=lc+1;i<rc;i++) res|=ch[i]->o;
...
void collect(Node* nd,vector<ll>& a){
    ...
    if(nd->ch[0]->ch.empty())
        for(auto c:nd->ch) a.push_back(c->o);
    else
...
void build(vector<ll>& a){
    ...
    function<void(Node*,int&)> dfs=[&](Node* nd,int& i){
        if(nd->ch.empty()) return;
        if(nd->ch[0]->ch.empty()){
            for(auto c:nd->ch) if(i<a.size()) c->o=a[i++];
            nd->calc(); nd->pref();
        }else{
