// MINIMUM VERSION: Ultra-Aggressive (4*T) with Binary Search
#include<bits/stdc++.h>
using namespace std;
...
struct Node{
    int sz;
    vector<Node*> ch;
    ll mn, lz;  // min instead of sum
    vector<ll> pm;  // prefix min
    vector<int> pz;
    
    Node(int n=0): sz(n), mn(INF), lz(0){}
    ...
    ll qry(int l,int r){
        if(ch.empty()||l>sz||r<1) return INF;
        l=max(1,l); r=min(r,sz);
        if(l>r) return INF;
        if(l==1&&r==sz) return mn;
        push();
        if(ch[0]->ch.empty()){
            ll res=INF;
            for(int i=l,e=min(r,(int)ch.size());i<=e;i++) res=min(res,ch[i-1]->mn);
            return res;
        }
        int lc=find(l),rc=find(r);
        if(lc==rc) return ch[lc]->qry(l-pz[lc],r-pz[lc]);
        ll res=ch[lc]->qry(l-pz[lc],ch[lc]->sz);
        for(int i=lc+1;i<rc;i++) res=min(res,ch[i]->mn);
        res=min(res,ch[rc]->qry(1,r-pz[rc]));
        return res;
    }
    ...
    void add(int l,int r,ll d){
        ...
        l=max(1,l); r=min(r,sz);
        if(l>r) return;
        if(l==1&&r==sz){ mn+=d; lz+=d; return; }
        push();
        if(ch[0]->ch.empty()){
            for(int i=l,e=min(r,(int)ch.size());i<=e;i++) ch[i-1]->mn+=d;
            calc(); pref(); return;
        }
        int lc=find(l),rc=find(r);
...
void collect(Node* nd,vector<ll>& a){
    ...
    nd->push();
    if(nd->ch[0]->ch.empty())
        for(auto c:nd->ch) a.push_back(c->mn);
    else
...
void build(vector<ll>& a){
    ...
    function<void(Node*,int&)> dfs=[&](Node* nd,int& i){
        if(nd->ch.empty()) return;
        if(nd->ch[0]->ch.empty()){
            for(auto c:nd->ch) if(i<a.size()) c->mn=a[i++];
            nd->calc(); nd->pref();
        }else{
